...
log('STARTUP: Application Startup Initiated')
...
log('STARTUP: Model Import Started')
from .models import *
log('STARTUP: Model Import Completed')

log('STARTUP: Database Build Started')
try:
    Base.metadata.create_all(engine, checkfirst=True)
    log('STARTUP: Database Build Completed')
except Exception as ex:
    log('STARTUP: Failed to create database tables with exception: ' % ex)

Session = sessionmaker(bind=engine)

log('STARTUP: Importing Setup Module')
from .mod_setup import setup
app.register_blueprint(setup)

log('STARTUP: Importing Home Module')
from .mod_home import home
app.register_blueprint(home)

log('STARTUP: Importing Lists Module')
from .mod_lists import lists
app.register_blueprint(lists)

log('STARTUP: Importing Requests Module')
from .mod_requests import requests
app.register_blueprint(requests)

log('STARTUP: Importing Admin Module')
...
try:
    if admin is None:
        ...
        if findadmin is not None:
            log('STARTUP: Found Administrator user did not match AdminUUID parameter. Updating the user details to match.')
            ...
            log('STARTUP: Welcome to the music camp scheduler! This is the first boot of the app. Look in your applicaiton parameters for the AdminUUID parameter, then log in to the setup page with websitename/user/AdminUUID(replace this with your admin UUID)/setup/')
    ...
    log('STARTUP: Failed to validate administrator with exception: ' % ex)
session.close()
session.flush()
